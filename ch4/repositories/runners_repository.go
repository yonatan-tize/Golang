...
func (rr RunnersRepository) UpdateRunner(runnerId string, runner *models.Runner)*models.ResponseError{
...
func (rr RunnersRepository) UpdateRunnerResults(runnerId string, runner *models.Runner) *models.ResponseError {
	query := `
		UPDATE runners
		SET
			personal_best = $1,
...
func (rr RunnersRepository) GetAllRunners()([]*models.Runner, *models.ResponseError){
	...
	for rows.Next(){
		...
		err := rows.Scan(&id, &firstName, &lastName, &age, &isActive,
			&country, &personalBest, &seasonBest) 
